...
module.exports = class SessionPersistence {
  ...
  async existsTodoListTitle(todoListTitle) {
    const FIND_TODOLIST_TITLE = `SELECT null FROM todolists WHERE title = $1 AND username = $2`;
  ...
  async setTodoListTitle(todoListId, todoListTitle) {
    ...
    let result = await dbQuery(SET_TODOLIST_TITLE, todoListTitle, todoListId, this.username);
  ...
  isUniqueConstraintViolation(error) {
    return /duplicate key value violates unique constaint/.test(String(error));
  }
};