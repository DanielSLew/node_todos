...
app.post("/lists/:todoListId/todos/:todoId/destroy", 
  ...
  catchError(async (req, res, next) => {
    ...
    let deletedTodo = await res.locals.store.deleteTodo(+todoListId, +todoId);
...
app.post("/lists/:todoListId/complete_all", 
  ...
  catchError(async (req, res, next) => {
    let todoListId = req.params.todoListId;
    if (!res.locals.signedIn) res.redirect(`/lists/${todoListId}`);    
    let markedAllDone = await res.locals.store.completeAllTodos(+todoListId);
